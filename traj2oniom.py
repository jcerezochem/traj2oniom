...
%s""" % (mem, nproc, chk, method, title, chargemult)
    ...
    for atom in atomsQMMM:
        ...
        if atom in atomsQM:
            if atomsMM:
                atomlabel = '%s' % (atom.name+'-'+atom.type +
                                    '-'+str(round(atom.charge, 5)))
            else:
                atomlabel = '%s' % (atom.name)
            print('%-20s %10.5f %10.5f %10.5f %s' %
                  (atomlabel, *atom.position, hl_flag), file=unit)
        # Low layer
        else:
            i1 = map_index[atom.index]
            if i1 in qmmm_bonds:
                ...
                ll_flag = 'L'
            print('%-20s %10.5f %10.5f %10.5f %s' % (atom.name+'-'+atom.type +
                  '-'+str(round(atom.charge, 5)), *atom.position, ll_flag), file=unit)
            ...
            for bonded_atom in atom.bonded_atoms:
                if bonded_atom.index in map_index:
                    ...
                    if atom in atomsQM.atoms:
                        print(
                            "WARNING: atom %s in QM layer bonded to PCsol layer" % atom.name)
                    # else:
                        #print("NOTE: atom %s in MM layer bonded to PCsol layer"%atom.name)
                else:
                    if atom in atomsQM.atoms:
                        print(
                            "WARNING: atom %s in QM layer has an external bond" % atom.name)
                    else:
                        print(
                            "WARNING: atom %s in MM layer has an external bond" % atom.name)
    ...
    if atomsPC:
        ...
        for atom in atomsPC:
            print('%10.5f %10.5f %10.5f %10.5f' %
                  (*atom.position, round(atom.charge, 5)), file=unit)
        print("", file=unit)
...
def write_oniom_gro(atomsQM, atomsMM, atomsPC, grofile, vmd_visualization=True):
    ...
    AllLayers = sels2ulayers([atomsQM, atomsMM, atomsPC], [
                             'QML', 'MML', 'PCL'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', metavar='file.trr',
                        help='Trajectory file', required=True)
    parser.add_argument('-f_fmt', metavar='trr',
                        help='Format of trajectory file', required=False)
    parser.add_argument('-s', metavar='file.tpr',
                        help='Binary topoly file', required=True)
    parser.add_argument('-s_fmt', metavar='tpr',
                        help='Format of topoly file', required=False)
    parser.add_argument('-selQM', metavar='select_string',
                        help='Selection command for the QM layer', default=None, required=True)
    parser.add_argument('-selMM', metavar='select_string',
                        help='Selection command for the MM layer', default=None)
    parser.add_argument('-selPC', metavar='select_string',
                        help='Selection command for the PC layer', default=None)
    parser.add_argument('-indQM', metavar='file.ndx',
                        help='Index file for the QM layer', default=None)
    parser.add_argument('-indMM', metavar='file.ndx',
                        help='Index file for the MM layer', default=None)
    parser.add_argument('-indPC', metavar='file.ndx',
                        help='Index file for the PC layer', default=None)
    parser.add_argument('-la', metavar='ATOM',
                        help='Linking atom label (e.g. H-H-0.1)', default='H-H-0.1')
    parser.add_argument(
    ...
    parser.add_argument('-b', metavar='<time>',
                        help='First frame (ps) to read from trajectory', type=float, default=-1.)
    parser.add_argument('-e', metavar='<time>',
                        help='Last frame (ps) to read from trajectory', type=float, default=-1.)
    parser.add_argument('-dt', metavar='<time>',
                        help='Only use frame when t MOD dt = first time (ps)', type=float, default=-1.)
    parser.add_argument(
        '-method', help='Whole route section (e.g. "#p hf/sto-3g")', default='#p hf/sto-3g')
    parser.add_argument(
        '-chargemult', help='Charge and multiplicity line (e.g. "0 1 0 1")', default=None)
    parser.add_argument('-computeQ', action='store_true',
                        help='Compute Charges in chargemult from topology', default=None)
    parser.add_argument('-FF', metavar='file.prm',
                        help='FF file into be added to Gaussian input', default=None)
    parser.add_argument('-writeGRO', action='store_true',
                        help='Write gro file to check layers', default=False)
    parser.add_argument('-nowrap', action='store_true',
                        help='Skip wrapping the layers get a compact representation', default=False)
    parser.add_argument('-compact', action='store_true',
                        help="Deprecated option to make system compact (now is the default)", default=False)
    parser.add_argument('-keep', action='store_true',
                        help='Keep atom ordering from trajectory (instead of reordering QM then MM)', default=False)
    parser.add_argument('-fixnames', action='store_true',
                        help='Try to convert atomnames into element names (WARNING: this might work unexpectedly)', default=False)
    # Parse input
    ...
    for conf in u.trajectory:
        ...
        write_oniom(layerQM, layerMM-layerQM, layerPC-layerMM-layerQM,
                    unit=f,
                    title='Trajectory step %s (time=%s ps)' % (
                        conf.frame, round(conf.time, 5)),
                    chk=chkname,
                    FFfile=args.FF,
                    method=args.method,
        ...
        if args.writeGRO:
            ...
            write_oniom_gro(layerQM, layerMM-layerQM, layerPC -
                            layerMM-layerQM, grofile, vmd_visualization=True)
